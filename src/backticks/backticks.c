...
static char *get_command_result(char *command, char ***env)
{
    ...
    size_t size = read(pipefd[0], content, 2048);
    content[size] = '\0';
    close(pipefd[0]);
    close(pipefd[1]);
    close(saved_stdout);
...
char *handle_backticks(char *user_input, char ***env)
{
    ...
    for (; *user_input; user_input++) {
        ...
        if (len_cmd == 0)
           continue;
        command_result = get_command_result(strndup(user_input, len_cmd), env);
        new_str = realloc(new_str, strlen(new_str) + strlen(command_result) + 1);
