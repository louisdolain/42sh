...
static void handle_printable(char *input, int *cursor_pos, int *input_length, char c)
{
...
static void handle_backspace(char *input, int *cursor_pos, int *input_length)
{
    if (*cursor_pos > 0) {
        memmove(input + (*cursor_pos) - 1, input + (*cursor_pos), *input_length - (*cursor_pos) + 1);
        printf("\b");
...
static void handle_delete_printable(char *input, int *cursor_pos, int *input_length, char c)
{
    if (c == 126) {
